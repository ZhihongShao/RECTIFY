...
class ShardedDataIterator(object):
    ...
    def __init__(
        ...
        self.num_extra_batches = math.ceil(samples_per_shard / batch_size) - math.ceil((self.shard_end_idx - self.shard_start_idx) / batch_size)
    ...
    # TODO: not used by MultiSetDataIterator defined below
    def iterate_ds_data(self, epoch: int = 0) -> Iterator[List]:
        ...
        # while self.iteration < max_iterations: # TODO: this original implementation has a bug --- `max_iterations` might be small than what it should be
        extra_batches = 0
        while extra_batches < self.num_extra_batches:
    ...
    def iterate_ds_sampled_data(
        ...
        # strict batch size, no matter what strict_batch_size is
        self.iteration = 0
        shard_indices = self.get_shard_indices(epoch)
