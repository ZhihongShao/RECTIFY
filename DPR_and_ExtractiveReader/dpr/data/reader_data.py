...
class ReaderPassage(object):
    ...
    def __init__(
        ...
        # rank
        self.passage_rank = passage_rank
        self.sub_doc_rank = sub_doc_rank
        # for evaluation when passage are split into sub docs
        self.token_is_max_context = defaultdict(lambda: True)
    ...
    def on_deserialize(self):
        ...
        self.segment_ids = torch.zeros_like(self.sequence_ids, dtype=torch.long)
        self.segment_ids[self.passage_offset:] = 1
...
class ExtractiveReaderDataset(torch.utils.data.Dataset):
    ...
    def _get_preprocessed_files(
        ...
        def _find_cached_files(path: str):
            ...
            cached_cfgs_str = _get_cached_data_cfgs_repr()
...
class SpanPrediction:
    def __init__(self, prediction_text="", start_logit=-1, end_logit=-1, span_score=-1, doc_score=-1, relevance_score=-1, passage_index=-1, subdoc_index=-1, passage_token_ids=None, score=-1):
        ...
        self.subdoc_index = subdoc_index
...
def set_default_reader_preprocessing_cfg(
    _truncated_ctx=True,
    _doc_stride=-1,
    _max_question_length=-1,
    _max_sequence_length=-1,
    _use_tailing_sep=False,
    _skip_no_positives=True,
    _include_gold_passage=False,
    _gold_page_only_positives=True,
    _max_positives=20,
    _max_negatives=50,
    _min_negatives=150,
    _max_retriever_passages=200
):
    global DEFAULT_PREPROCESSING_CFG_TRAIN
    DEFAULT_PREPROCESSING_CFG_TRAIN = DEFAULT_PREPROCESSING_CFG_TRAIN._replace(
        truncated_ctx=_truncated_ctx,
        doc_stride=_doc_stride,
        max_question_length=_max_question_length,
        max_sequence_length=_max_sequence_length,
        use_tailing_sep=_use_tailing_sep,
        skip_no_positves=_skip_no_positives,
        include_gold_passage=_include_gold_passage,
        gold_page_only_positives=_gold_page_only_positives,
        max_positives=_max_positives,
        max_negatives=_max_negatives,
        min_negatives=_min_negatives,
        max_retriever_passages=_max_retriever_passages,
    )

def _get_cached_data_cfgs_repr():
    global DEFAULT_PREPROCESSING_CFG_TRAIN
    cfg_str = "extractive_reader_prep_"
    for key in sorted(DEFAULT_PREPROCESSING_CFG_TRAIN._asdict()):
        val = getattr(DEFAULT_PREPROCESSING_CFG_TRAIN, key)
        cfg_str += "{}-{}_".format(key, val)
    return cfg_str[:-1]

def split_passage_into_overlapping_docs(
    sample, prefix_tensor, tensorizer, max_sequence_length, doc_stride, is_training, use_tailing_sep, sep_tensor
):
    prefix_token_ids = prefix_tensor.numpy().tolist()
    sep_token_ids = sep_tensor.numpy().tolist()
    all_doc_token_ids = sample.passage_token_ids.numpy().tolist()
    tok_to_orig_index = list(range(len(all_doc_token_ids)))
    samples = []

    max_tokens_for_doc = max_sequence_length - prefix_tensor.size(0) - int(use_tailing_sep) * sep_tensor.size(0)
    if doc_stride <= 0:
        doc_stride = max_tokens_for_doc
    ...
    doc_spans = []
    start_offset = 0
    while start_offset < len(all_doc_token_ids):
        length = len(all_doc_token_ids) - start_offset
        if length > max_tokens_for_doc:
            length = max_tokens_for_doc
        doc_spans.append(_DocSpan(start=start_offset, length=length))
        if start_offset + length == len(all_doc_token_ids):
            break
        start_offset += min(length, doc_stride)

    for (doc_span_index, doc_span) in enumerate(doc_spans):
        new_sample = copy.deepcopy(sample)
        token_ids = copy.deepcopy(prefix_token_ids)
        ...
        for i in range(doc_span.length):
            ...
            token_ids.append(all_doc_token_ids[split_token_index])
        new_sample.passage_token_ids = torch.tensor(token_ids[prefix_tensor.size(0):])
        new_sample.sequence_ids = torch.tensor(token_ids)

        if is_training and sample.has_answer:
            ...
            for span in sample.answers_spans:
                tok_start_pos, tok_end_pos = span
                ...
                answers_spans.append(
                    [tok_start_pos - doc_start, tok_end_pos - doc_start]
                )
            new_sample.answers_spans = answers_spans

        new_sample.token_is_max_context = token_is_max_context
        new_sample.sub_doc_rank = doc_span_index
...
def create_reader_sample_ids(
    ...
    use_tailing_sep: bool,
    is_train_set: bool,
) -> List[ReaderPassage]:
    question_for_concat = question
    question_token_ids = tensorizer.text_to_tensor(question_for_concat, add_special_tokens=False)
    if max_question_length > 0 and len(question_token_ids) > max_question_length:
        logger.warning("Question too long: {} truncated to {}".format(len(question_token_ids), max_question_length))
        question_for_concat = tensorizer.to_string(question_token_ids[:max_question_length])

    question_and_title = tensorizer.text_to_tensor(
        sample.title, title=question_for_concat, add_special_tokens=True
    )
    ...
    sep_tensor = tensorizer.get_pair_separator_ids()  # separator can be a multi tokens
    shift = question_and_title.size(0) + len(sep_tensor)
    assert shift > 1
    if truncated_ctx:
        all_concatenated, _ = _concat_pair(
            question_and_title,
        ...
        samples = split_passage_into_overlapping_docs(
            sample, question_and_title, tensorizer, max_sequence_length, doc_stride, is_train_set, use_tailing_sep, sep_tensor
        )
    for item in samples:
        item.passage_offset = shift
        if is_train_set:
            if item.has_answer:
                item.answers_spans = [
                    (span[0] + shift, span[1] + shift) for span in item.answers_spans
                ]
    return samples
...
def preprocess_retriever_data(
    ...
    for sample in samples:
        ...
        positive_passages = []
        for s in positive_passages:
            positive_passages.extend(create_reader_sample_ids(s, question))
        negative_passages = []
        for s in negative_passages:
            negative_passages.extend(create_reader_sample_ids(s, question))
...
def get_best_spans(
    ...
    doc_score: float,
    token_is_max_context: defaultdict,
    top_spans: int = 1,
) -> List[SpanPrediction]:
    """
    ...
    for (start_index, end_index), score in scores:
        ...
        best_spans.append(
            SpanPrediction(
                predicted_answer, start_logits[start_index], end_logits[end_index], score, doc_score, relevance_score, passage_idx, subdoc_idx, ctx_ids
            )
        )
