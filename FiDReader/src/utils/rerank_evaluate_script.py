...
def calculate_matches(
    ...
    for key in ['all', 'multi']:
        ...
        for ans_metric in ['f1', 'precision', 'recall', 'num', 'hit', 'pred']:
    ...
    if len(tot_multi) == 0:
        tot_multi.add(0)
    if len(tot_all) > 1 or len(tot_multi) > 1:
        print("WARNING > questions seem to have different number of supporting passages !!!", flush=True)
    # assert len(tot_all) == 1
    # assert len(tot_multi) == 1
    results['n_samples'] = max(list(tot_all))
    results['n_samples (multi)'] = max(list(tot_multi))
    if verbose:
        print(results)
    return results
...
def check_answer(inp, tokenizer, match_type, n_docs) -> Dict:
    ...
    for pred in preds:
        ...
        if pred not in all_preds and pred in all_answers:
            num_valid_pred += 1
            all_preds.add(pred)

    num_distinct_hit = 0
    all_answers = get_distinct_answers(sample)
    for answer in all_answers:
        if any(_normalize_answer(text) in all_preds for text in answer):
            num_distinct_hit += 1

    ret['all-ans-num'] = num_valid_pred
    ret['all-ans-hit'] = num_distinct_hit
    ret['all-ans-pred'] = len(preds)
    if is_multi:
        ret['multi-ans-num'] = num_valid_pred
        ret['multi-ans-hit'] = num_distinct_hit
        ret['multi-ans-pred'] = len(preds)
    return ret

def get_distinct_answers(sample):
    all_answers = []
    norm_ans_set = set()
    for anno in sample['annotations']:
        if anno['type'] == 'singleAnswer':
            ...
            answers = [qa['answer'] for qa in anno['qaPairs']]
        for answer in answers:
            tmp_norm_ans_set = set()
            for text in answer:
                tmp_norm_ans_set.add(_normalize_answer(text))
            if all(na not in norm_ans_set for na in tmp_norm_ans_set):
                all_answers.append(answer)
            for na in tmp_norm_ans_set:
                norm_ans_set.add(na)
    return all_answers
...
def locate_answer(answer, passage, tokenizer, uncased=True, ignore_puncts=False, ignore_stop_words=False):
...
def main():
    ...
    parser.add_argument("--src_filename", type=str, default="/home/shaozhihong/workspace/OpenDomainQA/FiD/open_domain_data/AmbigNQ/retrieved_dev.json")
