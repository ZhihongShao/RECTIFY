...
def embed_passages(opt, passages, model, tokenizer):
    ...
    if opt.is_realm_embedder:
        collator = src.data.TextCollatorForREALM(tokenizer, model.config.passage_maxlength)
        dataset = src.data.TextDatasetForREALM(passages)
    else:
    ...
    with torch.no_grad():
        for k, (ids, text_ids, text_mask, segment_ids) in enumerate(dataloader):
            embeddings = model.embed_text(
                text_ids=text_ids.cuda(),
                text_mask=text_mask.cuda(),
                token_type_ids=segment_ids.cuda() if opt.is_realm_embedder else None,
                apply_mask=model.config.apply_passage_mask,
                extract_cls=model.config.extract_cls,
                use_pooled_output=getattr(model.config, 'use_pooled_output', False),
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--is_realm_embedder',action='store_true')
