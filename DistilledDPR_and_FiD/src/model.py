...
class Retriever(transformers.PreTrainedModel):
    ...
    def __init__(self, config, initialize_wBERT=False):
        ...
        if self.config.projection:
            ...
            self.LayerNorm = nn.LayerNorm(self.config.indexing_dimension)
    ...
    def embed_text(self, text_ids, text_mask, token_type_ids=None, apply_mask=False, extract_cls=False, use_pooled_output=False):
        outputs = self.model(
            input_ids=text_ids,
        ...
        if type(outputs) is not tuple:
            outputs.to_tuple()
        text_output = outputs[1] if use_pooled_output else outputs[0]
        if self.config.projection:
            text_output = self.dense(text_output)
            text_output = self.LayerNorm(text_output)
        if not use_pooled_output:
            if extract_cls:
...
class BiEncoderRetriever(nn.Module):
    ...
    config_class = RetrieverConfig
    base_model_prefix = "biencoder_retriever"

    def __init__(self, config, q_embedder, ctx_embedder):
        super().__init__()
        self.config = config
        self.q_embedder = q_embedder
        self.ctx_embedder = ctx_embedder
        for embedder in [q_embedder, ctx_embedder]:
            assert hasattr(embedder, embed_text)

    def embed_q(self, text_ids, text_mask, token_type_ids=None):
        self.q_embedder(text_ids, text_mask, token_type_ids, self.config.apply_question_mask, self.config.extract_cls, self.config.use_pooled_output)
        
    def embed_ctx(self, text_ids, text_mask, token_type_ids=None):
        self.ctx_embedder(text_ids, text_mask, token_type_ids, self.config.apply_passage_mask, self.config.extract_cls, self.config.use_pooled_output)
