...
def embed_questions(opt, data, model, tokenizer):
    ...
    with torch.no_grad():
        for k, batch in enumerate(dataloader):
            (idx, _, _, question_ids, question_mask, segment_ids) = batch
            output = model.embed_text(
                ...
                token_type_ids=segment_ids.to(opt.device).view(-1, question_ids.size(-1)) if opt.is_realm_embedder else None,
